...
async def detect_anomalies(
    ...
    _: None = Depends(require_roles(["finance_team", "general_counsel", "cfo", "admin"]))
):
    """
    ...
    """
    try:
        logger.info(f"Anomaly detection requested by user {current_user.id} for company {request.company_id}")
        ...
        logger.info(f"Anomaly detection completed. Found {report.total_anomalies} anomalies")
        return response
...
async def get_anomaly_summary(
    ...
    """
    try:
        logger.info(f"Anomaly summary requested by user {current_user.id} for company {company_id}")
...
async def batch_detect_anomalies(
    ...
    """
    try:
        logger.info(f"Batch anomaly detection requested by user {current_user.id} for {len(request.company_ids)} companies")
        ...
        for company_id in request.company_ids:
            try:
                ...
                results.append(AnomalySummaryResponse(
                    company_id=company_id,
                    reporting_year=request.reporting_year,
                    total_anomalies=report.total_anomalies,
                    critical_anomalies=critical_count,
        ...
        logger.info(f"Batch anomaly detection completed. {successful_count}/{len(request.company_ids)} successful")
        return response
...
async def analyze_anomaly_trends(
    ...
    """
    try:
        logger.info(f"Anomaly trend analysis requested by user {current_user.id} for company {request.company_id}")
        ...
        for year in range(request.start_year, request.end_year + 1):
            try:
                ...
                for anomaly in report.detected_anomalies:
                    anomaly_type = anomaly.anomaly_type
                    if request.anomaly_types is None or anomaly_type in request.anomaly_types:
                        anomalies_by_type[anomaly_type] = anomalies_by_type.get(anomaly_type, 0) + 1
                
                trend_data.append({
                    "year": year,
                    "total_anomalies": len([a for a in report.detected_anomalies 
                                          if request.anomaly_types is None or a.anomaly_type in request.anomaly_types]),
                    "anomalies_by_type": anomalies_by_type,
                    "risk_score": report.overall_risk_score
                })
                
            except Exception as e:
                logger.warning(f"Could not analyze year {year} for company {request.company_id}: {str(e)}")
        ...
        trend_analysis = {
            ...
            "risk_score_trend": "increasing" if len(risk_scores) > 1 and risk_scores[-1] > risk_scores[0] else "stable",
            "total_anomalies_trend": "increasing" if len(total_anomalies) > 1 and total_anomalies[-1] > total_anomalies[0] else "stable",
        ...
        if trend_analysis["risk_score_trend"] == "increasing":
            recommendations.append("Risk score is increasing over time - implement systematic data quality improvements")
        if trend_analysis["total_anomalies_trend"] == "increasing":
            recommendations.append("Number of anomalies is increasing - review data collection and validation processes")
        if trend_analysis["average_risk_score"] > 50:
            recommendations.append("High average risk score - consider comprehensive data management system review")
...
async def compare_industry_benchmarks(
    ...
    """
    try:
        logger.info(f"Industry benchmark comparison requested by user {current_user.id} for company {request.company_id}")
        ...
        benchmarks = industry_benchmarks.get(industry_sector, industry_benchmarks["default"])
        ...
        for metric, deviation in deviations.items():
            if deviation > 0.2:
                recommendations.append(f"Consider improvement strategies for {metric} - currently {deviation:.1%} above industry average")
            elif deviation < -0.2:
                recommendations.append(f"Excellent performance in {metric} - {abs(deviation):.1%} below industry average")
