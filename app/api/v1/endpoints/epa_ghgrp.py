...
async def search_company_in_ghgrp(
    ...
    try:
        search_result = await ghgrp_service.search_company_in_ghgrp(
            ...
            search_criteria=additional_criteria
        )
...
async def get_ghgrp_emissions_data(
    company_id: str,
    reporting_year: int,
    ghgrp_facility_id: Optional[str] = Query(None, description="Specific GHGRP facility ID"),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        await audit_logger.log_event(
            ...
            details={
                ...
                "total_emissions": emissions_data.get("emissions_data", {}).get("total_emissions_co2e_tonnes", 0)
            }
        )
...
async def validate_calculation_against_ghgrp(
    calculation_id: str,
    reporting_year: int,
    company_id: Optional[str] = Query(None, description="Company ID (auto-detected if not provided)"),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    """
