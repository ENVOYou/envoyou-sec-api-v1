...
async def get_factors_summary(db: Session = Depends(get_db)):
    """
...
async def get_emission_factors(
    category: Optional[str] = Query(
        None, description="Filter by category (fuel, electricity)"
    ),
    fuel_type: Optional[str] = Query(None, description="Filter by fuel type"),
    electricity_region: Optional[str] = Query(
        None, description="Filter by electricity region"
    ),
    source: Optional[str] = Query(
        None, description="Filter by source (EPA_GHGRP, EPA_EGRID)"
    ),
...
async def get_emission_factor_by_code(
    ...
    version: Optional[str] = Query(
        None, description="Specific version, defaults to current"
    ),
    current_user: User = Depends(require_read_emissions),
    db: Session = Depends(get_db),
):
...
async def get_fuel_emission_factors(
    ...
    return epa_service.get_current_factors(category="fuel", fuel_type=request.fuel_type)
...
async def get_electricity_emission_factors(
    ...
    return epa_service.get_current_factors(
        category="electricity", electricity_region=request.region
    )
...
async def update_epa_factors(
    ...
    async with EPADataIngestionService(db) as epa_service:
        ...
        return epa_service.cache_with_versioning(
            data["factors"], request.source, version
        )
...
async def force_refresh_epa_data(
    ...
    return {"message": "EPA data refresh initiated", "results": result}


@router.get("/cache/status")
async def get_cache_status(current_user: User = Depends(require_manage_epa_data)):
    """
    ...
    return {"cache": cache_stats, "scheduler": scheduler_status}
...
async def get_calculations(
    ...
    scope: Optional[str] = Query(
        None, description="Filter by scope (scope_1, scope_2)"
    ),
    ...
    calculations = (
        query.order_by(EmissionsCalculation.created_at.desc())
        .offset(offset)
        .limit(limit)
        .all()
    )
...
@router.get(
    "/calculations/{calculation_id}", response_model=EmissionsCalculationResponse
)
...
@router.get(
    "/calculations/{calculation_id}/audit-trail",
    response_model=List[CalculationAuditTrailResponse],
)
...
async def get_forensic_report(
    ...
    include_user_details: bool = Query(
        False, description="Include user details (admin only)"
    ),
    current_user: User = Depends(require_read_emissions),
    db: Session = Depends(get_db),
):
    ...
    return audit_service.generate_forensic_report(
        calculation_id, include_raw_data, include_user_details
    )
...
async def get_company_emissions_summary(
    ...
    scope1_total = sum(
        c.total_co2e for c in calculations if c.scope == "scope_1" and c.total_co2e
    )
    scope2_total = sum(
        c.total_co2e for c in calculations if c.scope == "scope_2" and c.total_co2e
    )
    scope3_total = sum(
        c.total_co2e for c in calculations if c.scope == "scope_3" and c.total_co2e
    )

    # Calculate average data quality
    quality_scores = [
        c.data_quality_score for c in calculations if c.data_quality_score
    ]
    avg_quality = sum(quality_scores) / len(quality_scores) if quality_scores else None

    # Get last calculation date
    last_calc = (
        max([c.calculation_timestamp for c in calculations]) if calculations else None
    )
    ...
    return CompanyEmissionsSummary(
        ...
        total_co2e=(
            ...
            if any([scope1_total, scope2_total, scope3_total])
            else None
        ),
...
async def get_entities_with_emissions(
    ...
    include_consolidated: bool = Query(
        False, description="Include consolidated totals"
    ),
    current_user: User = Depends(require_read_emissions),
    db: Session = Depends(get_db),
):
    ...
    for entity in entities:
        ...
        if emissions:
            entity_data["emissions_data"] = {
                ...
                "total_co2e": (
                    float(emissions.total_co2e) if emissions.total_co2e else None
                ),
    ...
    result = {
        ...
        "entities_with_emissions": len(
            [e for e in entity_emissions if e["emissions_data"]]
        ),
...
async def trigger_consolidation_from_emissions(
    ...
    consolidation_method: str = Query(
        "ownership_based", description="Consolidation method"
    ),
    ...
    from app.schemas.consolidation import ConsolidationMethod, ConsolidationRequest
