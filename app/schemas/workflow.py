...
class WorkflowStepConfig(BaseModel):
    ...
    is_parallel: bool = Field(
        default=False, description="Can be executed in parallel with other steps"
    )
    timeout_hours: Optional[int] = Field(
        None, description="Hours before step times out"
    )
    auto_approve_conditions: Optional[Dict] = Field(
        None, description="Conditions for auto-approval"
    )
...
class WorkflowTemplateCreate(BaseModel):
    ...
    workflow_type: str = Field(
        ..., description="Type of workflow (report_approval, consolidation_approval)"
    )
    steps_config: List[WorkflowStepConfig] = Field(
...
class WorkflowCreate(BaseModel):
    ...
    due_date: Optional[datetime] = Field(
        None, description="Due date for workflow completion"
    )
...
class ApprovalRequestCreate(BaseModel):
    ...
    comments: Optional[str] = Field(
        None, description="Initial comments or instructions"
    )
...
class NotificationCreate(BaseModel):
    ...
    scheduled_at: Optional[datetime] = Field(
        None, description="When to send notification"
    )
...
class BulkApprovalRequest(BaseModel):
    ...
    approval_request_ids: List[UUID] = Field(
        ..., description="List of approval request IDs"
    )
...
class WorkflowSearchRequest(BaseModel):
    ...
    current_state: Optional[WorkflowState] = Field(
        None, description="Filter by current state"
    )
    ...
    date_to: Optional[datetime] = Field(
        None, description="Filter workflows created before this date"
    )
