...
class ActivityDataInput(BaseModel):
    ...
    activity_type: str = Field(..., description="Type of activity (fuel_combustion, electricity, etc.)")
    fuel_type: Optional[str] = Field(None, description="Type of fuel (for fuel-based activities)")
    ...
    activity_period_start: Optional[datetime] = Field(None, description="Start of activity period")
    activity_period_end: Optional[datetime] = Field(None, description="End of activity period")
    
    data_source: Optional[str] = Field(None, description="Source of the data")
    data_quality: Optional[str] = Field("measured", description="Quality of data (measured, calculated, estimated)")
    measurement_method: Optional[str] = Field(None, description="Method used to measure/calculate")
...
class Scope1CalculationRequest(BaseModel):
    ...
    entity_id: Optional[str] = Field(None, description="Entity UUID (optional for company-level)")
    ...
    activity_data: List[ActivityDataInput] = Field(..., min_items=1, description="List of activity data")
    
    calculation_parameters: Optional[Dict[str, Any]] = Field(None, description="Additional calculation parameters")
    source_documents: Optional[List[str]] = Field(None, description="References to source documents")
    ...
    def validate_scope1_activities(cls, v):
        valid_scope1_activities = [
            ...
            'fugitive_emissions', 'fuel_combustion'
        ]
        for activity in v:
            if activity.activity_type not in valid_scope1_activities:
                raise ValueError(f'Invalid Scope 1 activity type: {activity.activity_type}')
        return v
...
class Scope2CalculationRequest(BaseModel):
    ...
    electricity_consumption: List[ActivityDataInput] = Field(..., min_items=1, description="Electricity consumption data")
    
    calculation_method: str = Field("location_based", description="Calculation method (location_based, market_based)")
    calculation_parameters: Optional[Dict[str, Any]] = Field(None, description="Additional calculation parameters")
    source_documents: Optional[List[str]] = Field(None, description="References to source documents")
    notes: Optional[str] = Field(None, description="Calculation notes")
    ...
    def validate_scope2_activities(cls, v):
        for activity in v:
            if activity.activity_type != 'electricity_consumption':
                raise ValueError('Scope 2 calculations only accept electricity_consumption activities')
        return v
...
class CalculationExportRequest(BaseModel):
    ...
    include_audit_trail: bool = Field(False, description="Include audit trail in export")
