...
class EntityContribution(BaseModel):
    ...
    consolidation_factor: float = Field(..., ge=0.0, le=1.0, description="Factor applied for consolidation")
...
class ConsolidationRequest(BaseModel):
    ...
    include_scope3: bool = Field(default=False, description="Whether to include Scope 3 emissions")
    
    # Entity selection
    include_entities: Optional[List[UUID]] = Field(None, description="Specific entities to include (null = all active)")
    exclude_entities: Optional[List[UUID]] = Field(None, description="Entities to exclude")
    
    # Consolidation options
    minimum_ownership_threshold: float = Field(default=0.0, ge=0.0, le=100.0, description="Minimum ownership % to include")
    apply_operational_control_filter: bool = Field(default=False, description="Only include entities with operational control")
    
    # Data quality requirements
    minimum_data_quality_score: float = Field(default=0.0, ge=0.0, le=100.0)
    require_complete_data: bool = Field(default=False, description="Require complete data for all scopes")
