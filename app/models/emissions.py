...
class Company(BaseModel, AuditMixin):
    ...
    cik = Column(String(20), unique=True, index=True, nullable=True)  # SEC Central Index Key
    ...
    market_cap_category = Column(String(50), nullable=True)  # large-cap, mid-cap, small-cap
    
    # Relationships
    entities = relationship("CompanyEntity", back_populates="company", cascade="all, delete-orphan")
    calculations = relationship("EmissionsCalculation", back_populates="company", cascade="all, delete-orphan")
...
class CompanyEntity(BaseModel, AuditMixin):
    ...
    consolidation_method = Column(String(50), nullable=False)  # full, equity, proportional
    ...
    calculations = relationship("EmissionsCalculation", back_populates="entity", cascade="all, delete-orphan")
...
class EmissionsCalculation(BaseModel, AuditMixin):
    ...
    activity_data = relationship("ActivityData", back_populates="calculation", cascade="all, delete-orphan")
