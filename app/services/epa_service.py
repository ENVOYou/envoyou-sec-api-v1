...
from app.schemas.epa_data import (
    EmissionFactorCreate, EmissionFactorResponse, ValidationResult,
    EPADataUpdateRequest, EPADataUpdateResponse, EPAFactorSummary
)
...
class EPADataIngestionService:
    ...
    async def fetch_latest_factors(self, source: str = "EPA_GHGRP") -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Successfully fetched {len(data.get('factors', []))} factors from {source}")
    ...
    def validate_factor_data(self, factors: List[Dict[str, Any]]) -> ValidationResult:
        ...
        required_fields = [
            'factor_name', 'factor_code', 'category', 'unit',
            'co2_factor', 'co2e_factor', 'source', 'publication_year', 'version'
        ]
            ...
            try:
                ...
                if factor.get('electricity_region'):
                    ...
                    if factor['electricity_region'].lower() not in valid_regions:
                        warnings.append(f"Unknown electricity region: {factor['electricity_region']}")
    ...
    def cache_with_versioning(self, factors: List[Dict[str, Any]], source: str, version: str) -> EPADataUpdateResponse:
        """Cache EPA factors with versioning support"""
        try:
            logger.info(f"Caching {len(factors)} factors from {source} version {version}")
            ...
            existing_factors = self.db.query(EmissionFactor).filter(
                and_(
            ...
            for factor_data in factors:
                ...
                existing = self.db.query(EmissionFactor).filter(
                    and_(
            ...
            logger.info(f"Successfully cached EPA factors: {records_added} added, {records_updated} updated, {records_deprecated} deprecated")
    ...
    def get_current_factors(
        ...
        try:
            ...
            if electricity_region:
                query = query.filter(EmissionFactor.electricity_region == electricity_region)
    ...
    def get_factor_by_code(self, factor_code: str, version: Optional[str] = None) -> Optional[EmissionFactorResponse]:
    ...
    def get_factors_summary(self) -> EPAFactorSummary:
        ...
        try:
            ...
            current_factors = self.db.query(EmissionFactor).filter(
                EmissionFactor.is_current == True
            ).count()
            ...
            # Latest update
            latest_update = self.db.query(EPADataUpdate).filter(
                EPADataUpdate.status == "SUCCESS"
            ).order_by(desc(EPADataUpdate.created_at)).first()
            
            # Oldest factor
            oldest_factor = self.db.query(EmissionFactor).filter(
                EmissionFactor.is_current == True
            ).order_by(EmissionFactor.valid_from).first()
