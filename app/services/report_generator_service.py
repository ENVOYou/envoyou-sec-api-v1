...
class SECReportGenerator:
    ...
    async def generate_sec_report(
        ...
        if consolidation_id:
            consolidation = await self.consolidation_service.get_consolidation(consolidation_id)
        else:
            ...
            consolidation = await self.consolidation_service.get_consolidation(consolidations[0].id)

        # Build base report structure
        report_data = self._build_report_structure(consolidation, reporting_year, include_entity_breakdown)

        # Add audit trail if requested and user has permission
        if include_audit_trail and user and user.is_admin:
            report_data["audit_trail"] = await self._get_audit_trail_data(consolidation.id)
    ...
    def _build_report_structure(self, consolidation, reporting_year: int, include_entity_breakdown: bool) -> Dict[str, Any]:
        ...
        report = {
            ...
            "status": {
                ...
                "approved_at": consolidation.approved_at.isoformat() if consolidation.approved_at else None,
                "last_modified": consolidation.updated_at.isoformat() if consolidation.updated_at else None,
            },
        }
    ...
    def _format_scope1_table(self, consolidation) -> List[Dict[str, Any]]:
        ...
        for contrib in consolidation.entity_contributions:
            if contrib.consolidated_scope1_co2e and contrib.consolidated_scope1_co2e > 0:
        ...
        for fuel_type, emissions in fuel_totals.items():
            scope1_data.append({
                ...
                "percentage_of_total": round((emissions / consolidation.total_scope1_co2e * 100), 1) if consolidation.total_scope1_co2e else 0,
            })
    ...
    def _format_scope2_table(self, consolidation) -> List[Dict[str, Any]]:
        ...
        region_totals = {}
        for contrib in consolidation.entity_contributions:
            if contrib.consolidated_scope2_co2e and contrib.consolidated_scope2_co2e > 0:
        ...
        for region, emissions in region_totals.items():
            scope2_data.append({
                ...
                "emissions_mtco2e": round(emissions, 2),
                "percentage_of_total": round((emissions / consolidation.total_scope2_co2e * 100), 1) if consolidation.total_scope2_co2e else 0,
            })
        ...
        for contrib in consolidation.entity_contributions:
            if contrib.included_in_consolidation:
                segment_data.append({
                    ...
                    "scope1_emissions": round(contrib.consolidated_scope1_co2e or 0, 2),
                    "scope2_emissions": round(contrib.consolidated_scope2_co2e or 0, 2),
                    "scope3_emissions": round(contrib.consolidated_scope3_co2e or 0, 2),
                    "total_emissions": round(contrib.consolidated_total_co2e or 0, 2),
    ...
    async def _generate_pdf_report(self, report_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'emissions_tables' in report_data:
            ...
            if report_data['emissions_tables']['table_1_scope1_emissions']['data']:
                story.append(Paragraph("Scope 1 GHG Emissions by Source", styles['Heading3']))
                scope1_data = report_data['emissions_tables']['table_1_scope1_emissions']['data']
    ...
    async def _generate_excel_report(self, report_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'emissions_tables' in report_data:
            # Scope 1 Table
            if report_data['emissions_tables']['table_1_scope1_emissions']['data']:
                ws.cell(row=row, column=1, value="Scope 1 GHG Emissions by Source").font = header_font
                ws.merge_cells(start_row=row, start_column=1, end_row=row, end_column=3)
                row += 2

                # Headers
                ws.cell(row=row, column=1, value="Source Category").font = subheader_font
                ws.cell(row=row, column=2, value="Emissions (MTCO2e)").font = subheader_font
                ws.cell(row=row, column=3, value="Percentage").font = subheader_font
                row += 1

                # Data
                for data_row in report_data['emissions_tables']['table_1_scope1_emissions']['data']:
                    ws.cell(row=row, column=1, value=data_row['source_category']).font = normal_font
                    ws.cell(row=row, column=2, value=data_row['emissions_mtco2e']).font = normal_font
                    ws.cell(row=row, column=3, value=f"{data_row['percentage_of_total']:.1f}%").font = normal_font
                    row += 1
