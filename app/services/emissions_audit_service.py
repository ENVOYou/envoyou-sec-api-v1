...
from app.schemas.emissions import (
    CalculationAuditTrailResponse, EmissionsCalculationResponse
)
...
class EmissionsAuditService:
    ...
    def log_calculation_event(
        ...
        try:
            ...
            logger.info(f"Audit event logged: {event_type} for calculation {calculation_id} by user {user_id}")
    ...
    def get_calculation_audit_trail(
        ...
        try:
            ...
            if start_date:
                query = query.filter(CalculationAuditTrail.event_timestamp >= start_date)
            ...
            audit_entries = query.order_by(CalculationAuditTrail.event_timestamp.desc()).all()
    ...
    def get_company_audit_summary(
        ...
        try:
            ...
            compliance_status = self._assess_compliance_status(calculations, audit_events)
            ...
            return {
                ...
                "audit_coverage": self._calculate_audit_coverage(calculations, audit_events),
                "last_audit_activity": max([e.event_timestamp for e in audit_events]) if audit_events else None
            }
    ...
    def verify_calculation_integrity(self, calculation_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            audit_events = self.db.query(CalculationAuditTrail).filter(
                CalculationAuditTrail.calculation_id == calculation_id
            ).order_by(CalculationAuditTrail.event_timestamp).all()
            ...
            integrity_checks = {
                ...
                "audit_trail_complete": self._verify_audit_trail_completeness(calculation, audit_events),
                "data_consistency": self._verify_data_consistency(calculation, activity_data),
                "calculation_reproducible": self._verify_calculation_reproducibility(calculation, activity_data),
                "emission_factors_traceable": self._verify_emission_factors_traceability(activity_data),
                "timestamps_consistent": self._verify_timestamp_consistency(calculation, audit_events),
                "user_authorization_valid": self._verify_user_authorization(audit_events)
            }
            
            # Calculate overall integrity score
            passed_checks = sum(1 for check in integrity_checks.values() if check is True)
            total_checks = len(integrity_checks)
            integrity_score = (passed_checks / total_checks) * 100
            
            # Identify issues
            issues = [
                check_name for check_name, passed in integrity_checks.items() 
                if passed is False
            ]
        ...
        try:
            ...
            if calculation.entity_id:
                entity = self.db.query(CompanyEntity).filter(
                    CompanyEntity.id == calculation.entity_id
                ).first()
            ...
            forensic_report = {
                ...
                "entity_information": {
                    ...
                    "ownership_percentage": entity.ownership_percentage if entity else None
                } if entity else None,
                "audit_trail": {
                    ...
                    "activities": [
                        {
                            ...
                            "emission_factor": {
                                "id": str(ad.emission_factor_id) if ad.emission_factor_id else None,
                ...
                "compliance_attestation": {
                    ...
                    "audit_trail_complete": integrity_check["integrity_checks"]["audit_trail_complete"],
                    "data_traceable": integrity_check["integrity_checks"]["emission_factors_traceable"],
                    "calculation_reproducible": integrity_check["integrity_checks"]["calculation_reproducible"]
                }
            }
    ...
    def _calculate_data_quality_trends(self, calculations: List[EmissionsCalculation]) -> List[Dict[str, Any]]:
        ...
        for month, calcs in monthly_data.items():
            quality_scores = [c.data_quality_score for c in calcs if c.data_quality_score]
            ...
            trends.append({
                ...
                "completed_calculations": len([c for c in calcs if c.status == "completed"])
            })
    ...
    def _assess_compliance_status(
        ...
        incomplete_calcs = [c for c in calculations if c.status not in ["completed", "approved"]]
        if incomplete_calcs:
            return "pending"
    ...
    def _verify_emission_factors_traceability(self, activity_data: List[ActivityData]) -> bool:
    ...
    def _verify_user_authorization(self, audit_events: List[CalculationAuditTrail]) -> bool:
