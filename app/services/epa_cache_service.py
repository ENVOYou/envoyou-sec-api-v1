...
class EPACachedService:
    ...
    async def get_emission_factors(
        ...
        try:
            ...
            if not force_refresh:
                ...
                if cached_factors:
                    logger.debug(f"Returning {len(cached_factors)} cached factors for {source}")
                    return [EmissionFactorResponse(**factor) for factor in cached_factors]
        ...
        try:
            ...
            if db_factor:
                ...
                self.cache_service.cache.set_with_ttl(
                    self.cache_service._get_cache_key("factor", factor_code, version=version or "current"),
                    db_factor.dict(),
    ...
    async def refresh_epa_data(
        ...
        for source in sources:
            try:
                ...
                try:
                    ...
                    logger.info(f"Successfully refreshed {source}: {update_response.records_added} added, {update_response.records_updated} updated")
        ...
        return {
            ...
            "sources": results,
            "overall_status": "success" if all(r.get("status") in ["success", "skipped"] for r in results.values()) else "partial_failure"
        }
        ...
        try:
            ...
            if metadata:
                ...
                if age_hours < 168:  # 7 days
                    cache_key = self.cache_service._get_cache_key("factors", source, version="current")
                    extended_ttl = 24 * 3600  # Extend by 24 hours
                    
                    cached_factors = self.cache_service.cache.get(cache_key)
                    if cached_factors:
                        self.cache_service.cache.set_with_ttl(cache_key, cached_factors, extended_ttl)
                        logger.info(f"Extended cache TTL for {source} by 24 hours due to API unavailability")
                        return
    ...
    async def start_auto_refresh(self):
        ...
        logger.info(f"Starting auto-refresh task (interval: {self.refresh_interval_hours} hours)")
        ...
        while True:
            try:
                ...
                if refresh_results["overall_status"] == "success":
                    ...
                    logger.warning(f"Scheduled EPA data refresh completed with issues: {refresh_results}")
    ...
    def get_cache_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            service_status = {
                "auto_refresh_enabled": self._refresh_task is not None and not self._refresh_task.done(),
