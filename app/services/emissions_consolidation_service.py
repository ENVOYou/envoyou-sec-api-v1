...
class EmissionsConsolidationService:
    ...
    async def create_consolidation(
        ...
        try:
            ...
            consolidated_totals = self._calculate_consolidated_totals(
            ...
            version = await self._get_next_version(
                request.company_id, request.reporting_year
            )
            ...
            consolidation = ConsolidatedEmissions(
                ...
                entities_with_scope1=sum(
                    1
                    for c in filtered_contributions
                    if c.consolidated_scope1_co2e and c.consolidated_scope1_co2e > 0
                ),
                entities_with_scope2=sum(
                    1
                    for c in filtered_contributions
                    if c.consolidated_scope2_co2e and c.consolidated_scope2_co2e > 0
                ),
                entities_with_scope3=sum(
                    1
                    for c in filtered_contributions
                    if c.consolidated_scope3_co2e and c.consolidated_scope3_co2e > 0
                ),
                # Quality metrics
            ...
            response = ConsolidationDetailResponse(
                ...
                total_scope1_co2e=(
                    float(consolidation.total_scope1_co2e)
                    if consolidation.total_scope1_co2e
                    else None
                ),
                total_scope2_co2e=(
                    float(consolidation.total_scope2_co2e)
                    if consolidation.total_scope2_co2e
                    else None
                ),
                total_scope3_co2e=(
                    float(consolidation.total_scope3_co2e)
                    if consolidation.total_scope3_co2e
                    else None
                ),
                total_co2e=(
                    float(consolidation.total_co2e)
                    if consolidation.total_co2e
                    else None
                ),
                total_co2=(
                    float(consolidation.total_co2) if consolidation.total_co2 else None
                ),
                total_ch4_co2e=(
                    float(consolidation.total_ch4_co2e)
                    if consolidation.total_ch4_co2e
                    else None
                ),
                total_n2o_co2e=(
                    float(consolidation.total_n2o_co2e)
                    if consolidation.total_n2o_co2e
                    else None
                ),
                total_other_ghg_co2e=(
                    float(consolidation.total_other_ghg_co2e)
                    if consolidation.total_other_ghg_co2e
                    else None
                ),
                ...
                data_completeness_score=(
                    float(consolidation.data_completeness_score)
                    if consolidation.data_completeness_score
                    else None
                ),
                consolidation_confidence_score=(
                    float(consolidation.consolidation_confidence_score)
                    if consolidation.consolidation_confidence_score
                    else None
                ),
            ...
            logger.info(
                f"Created consolidation {consolidation.id} for company {request.company_id}"
            )
            return response
    ...
    async def _get_entities_for_consolidation(
        ...
        if request.minimum_ownership_threshold > 0:
            query = query.filter(
                CompanyEntity.ownership_percentage
                >= request.minimum_ownership_threshold
            )
    ...
    async def _get_entity_emissions_data(
        ...
        logger.info(
            f"Found emissions data for 0 entities out of {len(entities)} total entities"
        )
        return {}
    ...
    async def _calculate_entity_contributions(
        ...
        for entity in entities:
            ...
            consolidation_factor = self._get_consolidation_factor(
                entity, consolidation_method
            )

            # Calculate data quality metrics
            data_completeness = (
                self._calculate_data_completeness(emissions) if emissions else 0.0
            )
            data_quality_score = (
                self._calculate_data_quality_score(emissions) if emissions else 0.0
            )
            ...
            contribution = EntityContribution(
                ...
                original_scope1_co2e=(
                    float(emissions.total_scope1_co2e)
                    if emissions and emissions.total_scope1_co2e
                    else None
                ),
                original_scope2_co2e=(
                    float(emissions.total_scope2_co2e)
                    if emissions and emissions.total_scope2_co2e
                    else None
                ),
                original_scope3_co2e=(
                    float(emissions.total_scope3_co2e)
                    if emissions and emissions.total_scope3_co2e
                    else None
                ),
                original_total_co2e=(
                    float(emissions.total_co2e)
                    if emissions and emissions.total_co2e
                    else None
                ),
                # Consolidated contributions (original * consolidation_factor)
                consolidated_scope1_co2e=(
                    float(emissions.total_scope1_co2e) * consolidation_factor
                    if emissions and emissions.total_scope1_co2e
                    else None
                ),
                consolidated_scope2_co2e=(
                    float(emissions.total_scope2_co2e) * consolidation_factor
                    if emissions and emissions.total_scope2_co2e
                    else None
                ),
                consolidated_scope3_co2e=(
                    float(emissions.total_scope3_co2e) * consolidation_factor
                    if emissions and emissions.total_scope3_co2e
                    else None
                ),
                consolidated_total_co2e=(
                    float(emissions.total_co2e) * consolidation_factor
                    if emissions and emissions.total_co2e
                    else None
                ),
    ...
    def _get_consolidation_factor(
        self, entity: CompanyEntity, method: ConsolidationMethod
    ) -> float:
    ...
    def _apply_consolidation_filters(
        ...
        for contribution in contributions:
            ...
            if (
                not contribution.consolidated_scope1_co2e
                and not contribution.consolidated_scope2_co2e
                and (
    ...
    def _calculate_consolidated_totals(
        self, contributions: List[EntityContribution]
    ...
    def _calculate_quality_metrics(
        ...
        included_contributions = [
            c for c in contributions if c.included_in_consolidation
        ]
        ...
        return {"completeness": weighted_completeness, "confidence": confidence}
    ...
    async def get_consolidation(
        ...
        if consolidation.entity_contributions:
            for entity_id, contrib_data in consolidation.entity_contributions.items():
                entity_contributions.append(
                    EntityContribution(entity_id=UUID(entity_id), **contrib_data)
                )
        ...
        return ConsolidationDetailResponse(
            ...
            total_co2e=(
                float(consolidation.total_co2e) if consolidation.total_co2e else None
            ),
            total_co2=(
                float(consolidation.total_co2) if consolidation.total_co2 else None
            ),
    ...
    async def list_consolidations(
        ...
        return [
            ConsolidationResponse(
                ...
                total_scope1_co2e=(
                    float(c.total_scope1_co2e) if c.total_scope1_co2e else None
                ),
                total_scope2_co2e=(
                    float(c.total_scope2_co2e) if c.total_scope2_co2e else None
                ),
                total_scope3_co2e=(
                    float(c.total_scope3_co2e) if c.total_scope3_co2e else None
                ),
                ...
                data_completeness_score=(
                    float(c.data_completeness_score)
                    if c.data_completeness_score
                    else None
                ),
                consolidation_confidence_score=(
                    float(c.consolidation_confidence_score)
                    if c.consolidation_confidence_score
                    else None
                ),
    ...
    async def get_consolidation_summary(
        ...
        # Count by status
        approved_count = sum(
            1 for c in consolidations if c.status == ConsolidationStatus.APPROVED.value
        )
        draft_count = sum(
            1 for c in consolidations if c.status == ConsolidationStatus.DRAFT.value
        )

        # Get total entities in company structure
        total_entities = (
            self.db.query(func.count(CompanyEntity.id))
            .filter(CompanyEntity.company_id == company_id)
            .scalar()
        ) or 0

        coverage_percentage = (
            (latest.total_entities_included / total_entities * 100.0)
            if total_entities > 0
            else 0.0
        )
        ...
        return ConsolidationSummary(
            ...
            latest_scope1_co2e=(
                float(latest.total_scope1_co2e) if latest.total_scope1_co2e else None
            ),
            latest_scope2_co2e=(
                float(latest.total_scope2_co2e) if latest.total_scope2_co2e else None
            ),
            latest_scope3_co2e=(
                float(latest.total_scope3_co2e) if latest.total_scope3_co2e else None
            ),
