...
class EPACacheService:
    ...
    def cache_emission_factors(
        ...
        try:
            ...
            success = self.cache.set_with_ttl(all_factors_key, factors, self.default_ttl)
            ...
            for factor in factors:
                ...
                if factor_code:
                    factor_key = self._get_cache_key("factor", factor_code, version=version)
    ...
    def get_emission_factors(
        ...
        try:
            ...
            cache_key = self._get_cache_key("factors", source, version=version or "current")
    ...
    def get_emission_factor_by_code(
        ...
        try:
            cache_key = self._get_cache_key("factor", factor_code, version=version or "current")
    ...
    def get_cache_metadata(self, source: str, version: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """Get cache metadata for EPA data"""
        try:
            metadata_key = self._get_cache_key("metadata", source, version=version or "current")
    ...
    def invalidate_source_cache(self, source: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Invalidated {deleted_count} cache entries for source: {source}")
    ...
    def get_cache_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for source in sources:
                ...
                if metadata:
                    cache_key = self._get_cache_key("factors", source, version="current")
                    ttl = self.cache.get_ttl(cache_key)
