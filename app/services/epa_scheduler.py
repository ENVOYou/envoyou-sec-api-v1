...
class EPADataScheduler:
    ...
    async def start_scheduler(self):
        ...
        logger.info(f"Starting EPA data scheduler with {self.refresh_interval_hours}h interval")
    ...
    async def _refresh_source_with_retry(self, epa_service: EPADataIngestionService, source: str):
        """Refresh EPA data source with retry logic"""
        for attempt in range(self.max_retries):
            try:
                logger.info(f"Refreshing {source} (attempt {attempt + 1}/{self.max_retries})")
                ...
                if data and data.get('factors'):
                    ...
                    version = data.get('metadata', {}).get('version', f"auto_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}")
                    ...
                    cache_service.set_cache_staleness_indicator(f"epa_factors_{source.lower()}", False)
                    
                    logger.info(f"Successfully refreshed {source}: {update_result.records_added} added, {update_result.records_updated} updated")
                    break  # Success, exit retry loop
                else:
                    logger.warning(f"No data received from {source}")
                    
            except Exception as e:
                logger.error(f"Failed to refresh {source} (attempt {attempt + 1}): {str(e)}")
                ...
                if attempt < self.max_retries - 1:
                    ...
                    cache_service.set_cache_staleness_indicator(f"epa_factors_{source.lower()}", True)
                    logger.error(f"All retry attempts failed for {source}, marking cache as stale")
        ...
        try:
            ...
            for category, category_factors in categories.items():
                ...
                success = cache_service.set_epa_factors(
                    cache_key,
                    category_factors,
                    settings.EPA_DATA_CACHE_HOURS
                )
                
                if success:
                    logger.debug(f"Updated Redis cache for {cache_key}: {len(category_factors)} factors")
    ...
    async def _update_cache_staleness(self):
        ...
        try:
            ...
            sources = ["EPA_GHGRP", "EPA_EGRID"]
            staleness_threshold = timedelta(hours=self.refresh_interval_hours * 2)  # 2x the refresh interval
            
            for source in sources:
                last_update = db.query(EPADataUpdate).filter(
                    EPADataUpdate.source == source,
                    EPADataUpdate.status == "SUCCESS"
                ).order_by(EPADataUpdate.created_at.desc()).first()
                ...
                if last_update:
                    ...
                    cache_service.set_cache_staleness_indicator(
                        f"epa_factors_{source.lower()}",
                        is_stale
                    )
                    
                    if is_stale:
                        logger.warning(f"{source} data is stale (last update: {last_update.created_at})")
                else:
                    ...
                    logger.warning(f"No successful updates found for {source}, marking as stale")
            ...
            async with EPADataIngestionService(db) as epa_service:
                ...
                for src in sources_to_refresh:
                    try:
                        ...
                        if data and data.get('factors'):
                            version = f"force_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"
                            ...
                            # Clear staleness indicator
                            cache_service.set_cache_staleness_indicator(f"epa_factors_{src.lower()}", False)
