...
class EPAGHGRPService:
    ...
    async def search_company_in_ghgrp(
        self, company_id: str, search_criteria: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self.cache_service.cache.set_with_ttl(
                cache_key, search_result, ttl_seconds=24 * 3600  # 24 hours
            )

            logger.info(
                f"GHGRP search completed for company {company_id}: {len(ghgrp_matches)} matches found"
            )
    ...
    async def get_ghgrp_emissions_data(
        ...
        try:
            ...
            emissions_data = await self._get_ghgrp_emissions_api(
            ...
            self.cache_service.cache.set_with_ttl(
                cache_key, result, ttl_seconds=7 * 24 * 3600  # 7 days
            )

            logger.info(
                f"GHGRP emissions data retrieved for company {company_id}, year {reporting_year}"
            )
    ...
    async def validate_company_emissions(
        self, company_id: str, calculation_id: str, reporting_year: int
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            validation_result = self._compare_emissions_data(
                calculation, ghgrp_data["emissions_data"]
            )

            # Calculate discrepancy analysis
            discrepancy_analysis = self._analyze_discrepancies(
                calculation, ghgrp_data["emissions_data"], validation_result
            )

            # Generate recommendations
            recommendations = self._generate_validation_recommendations(
                validation_result, discrepancy_analysis
            )
            ...
            final_result = {
                ...
                "confidence_level": self._calculate_validation_confidence(
                    validation_result, ghgrp_data["data_quality"]
                ),
            }
    ...
    def _build_search_parameters(
        self, company: Company, additional_criteria: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
    ...
    async def _simulate_ghgrp_search_response(
        self, search_params: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    def _rank_company_matches(
        self, company: Company, ghgrp_matches: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        ...
        for match in ghgrp_matches:
            ...
            if company.name and match.get("parent_company"):
                name_similarity = self._calculate_name_similarity(
                    company.name, match["parent_company"]
                )
            ...
            if company.industry and match.get("sector"):
                sector_match = self._match_industry_to_sector(
                    company.industry, match["sector"]
                )
    ...
    async def _get_ghgrp_emissions_api(
        self, facility_id: str, reporting_year: int
    ) -> Dict[str, Any]:
        """Get emissions data from GHGRP API"""
    ...
    def _process_ghgrp_emissions_data(self, raw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        processed = {
            ...
            "scope_1_emissions": {
                "total_co2e_tonnes": emissions_data.get("scope_1", {}).get(
                    "total_co2e", 0
                ),
                ...
                "emission_sources": emissions_data.get("scope_1", {}).get(
                    "sources", {}
                ),
            },
            "scope_2_emissions": {
                "total_co2e_tonnes": emissions_data.get("scope_2", {}).get(
                    "total_co2e", 0
                ),
                "electricity_mwh": emissions_data.get("scope_2", {}).get(
                    "electricity_purchased", 0
                ),
                "steam_mmbtu": emissions_data.get("scope_2", {}).get(
                    "steam_purchased", 0
                ),
                "electricity_emission_factor": emissions_data.get("scope_2", {}).get(
                    "emission_factor_electricity", 0
                ),
            },
    ...
    def _assess_ghgrp_data_quality(
        ...
        quality_assessment = {
            ...
            "verification_status": quality_metrics.get(
    ...
    def _analyze_scope_coverage(self, processed_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        scope_coverage = {
            "scope_1_available": processed_data.get("scope_1_emissions", {}).get(
                "total_co2e_tonnes", 0
            )
            > 0,
            "scope_2_available": processed_data.get("scope_2_emissions", {}).get(
                "total_co2e_tonnes", 0
            )
            > 0,
            ...
            "emission_sources": {
                ...
                "process_emissions": bool(
                    processed_data.get("scope_1_emissions", {})
                    .get("emission_sources", {})
                    .get("process_emissions")
                ),
                "electricity_purchased": "scope_2_emissions" in processed_data,
                "steam_purchased": bool(
                    processed_data.get("scope_2_emissions", {}).get("steam_mmbtu", 0)
                    > 0
                ),
            },
        ...
        available_scopes = sum(
            [scope_coverage["scope_1_available"], scope_coverage["scope_2_available"]]
        )
        scope_coverage["coverage_completeness"] = (
            available_scopes / 2
        ) * 100  # Out of Scope 1 & 2

        return scope_coverage

    def _compare_emissions_data(
        self, calculation: EmissionsCalculation, ghgrp_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if calculation.scope == "scope_1" or calculation.total_co2e:
            ...
            ghgrp_scope1 = ghgrp_data.get("scope_1_emissions", {}).get(
                "total_co2e_tonnes", 0
            )
        ...
        if calculation.scope == "scope_2":
            ...
            ghgrp_scope2 = ghgrp_data.get("scope_2_emissions", {}).get(
                "total_co2e_tonnes", 0
            )
        ...
        if comparison_result["scope_1_comparison"]:
            total_variance_statuses.append(
                comparison_result["scope_1_comparison"]["status"]
            )
        if comparison_result["scope_2_comparison"]:
            total_variance_statuses.append(
                comparison_result["scope_2_comparison"]["status"]
            )
        ...
        if total_variance_statuses:
            if all(status == "acceptable" for status in total_variance_statuses):
                ...
                status == "significant_variance" for status in total_variance_statuses
            ):
    ...
    def _analyze_discrepancies(
        ...
        ghgrp_quality = ghgrp_data.get("data_quality_metrics", {}).get(
            "completeness", 0
        )
        ...
        # Temporal factors
        calc_year = (
            calculation.reporting_period_start.year
            if calculation.reporting_period_start
            else None
        )
    ...
    def _generate_validation_recommendations(
        self, validation_result: Dict[str, Any], discrepancy_analysis: Dict[str, Any]
    ) -> List[str]:
        ...
        if "Low data quality" in str(
            discrepancy_analysis.get("data_quality_factors", [])
        ):
            recommendations.append(
                "📈 Improve data collection processes and quality controls"
            )
    ...
    def _calculate_validation_confidence(
        self, validation_result: Dict[str, Any], ghgrp_data_quality: Dict[str, Any]
    ) -> float:
    ...
    async def get_ghgrp_validation_summary(
        self, company_id: str, reporting_year: Optional[int] = None
    ) -> Dict[str, Any]:
