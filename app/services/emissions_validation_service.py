...
class EmissionsValidationService:
    ...
    async def validate_company_emissions(
        ...
        try:
            ...
            if not company_emissions["calculations"]:
                ...
                result.recommendations.append("No emissions calculations found for validation")
                return result
            ...
            confidence_scores = self._calculate_confidence_scores(
                company_emissions, ghgrp_validation, variance_analysis, threshold_analysis
            )
            ...
            result.validation_status, result.compliance_level = self._determine_validation_status(
                ...
                return {
                    **base_report,
                    "executive_summary": {
                        ...
                        "confidence_level": self._get_confidence_level(validation_result.overall_confidence_score),
    ...
    async def _perform_ghgrp_cross_validation(
        ...
        try:
            ...
            return {
                ...
                "variance_percentage": ghgrp_results.get("summary", {}).get("difference_percentage", 0),
        ...
        try:
            ...
            for scope, total in company_emissions["scope_totals"].items():
                ...
                scope_variances[scope] = {
                    "company_total": total,
                    "variance_from_total": (total / company_total * 100) if company_total > 0 else 0
                }
            ...
            return {
                ...
                "variance_direction": "higher" if company_total > ghgrp_total else "lower"
            }
    ...
    def _perform_threshold_analysis(self, variance_analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "percentage_variance": percentage_variance,
                "threshold_exceeded": percentage_variance > self.variance_thresholds["low"],
                "thresholds": self.variance_thresholds,
                "risk_assessment": self._assess_threshold_risk(threshold_level, percentage_variance)
            }
    ...
    def _calculate_confidence_scores(
        ...
        try:
            ...
            ghgrp_score = 100.0 if ghgrp_validation.get("data_available", False) else 0.0
            ...
            if variance_analysis.get("variance_available", False):
                ...
                variance_score = max(0, 100 - (variance_pct * 2))  # 2% penalty per 1% variance
            
            # Data quality score
            calculation_count = company_emissions["calculation_count"]
            data_quality_score = min(100.0, calculation_count * 20)  # 20 points per calculation, max 100
            
            # Completeness score
            scope_count = len(company_emissions["scope_totals"])
            completeness_score = min(100.0, scope_count * 50)  # 50 points per scope, max 100
    ...
    def _determine_validation_status(
        ...
        try:
            ...
            if overall_score >= 85 and not critical_discrepancies and not high_discrepancies:
    ...
    def _generate_recommendations(
        ...
        try:
            ...
            if critical_discrepancies:
                recommendations.append("Address critical discrepancies immediately before SEC filing")
            ...
            if result.completeness_score < 80:
                recommendations.append("Ensure all emission scopes are properly calculated and documented")
            
            if result.consistency_score < 80:
                recommendations.append("Review calculation methodology for consistency with EPA standards")
            
            # General recommendations
            recommendations.append("Maintain comprehensive audit trail for all emissions calculations")
            recommendations.append("Regular validation against EPA GHGRP database recommended")
    ...
    def _assess_threshold_risk(self, threshold_level: str, percentage_variance: float) -> Dict[str, Any]:
