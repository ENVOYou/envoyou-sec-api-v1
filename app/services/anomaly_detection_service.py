...
class AnomalyDetectionService:
    ...
    def detect_anomalies(
        ...
        """
        logger.info(f"Starting anomaly detection for company {company_id}, year {reporting_year}")
        
        try:
            # Get company's emissions data
            current_year_data = self._get_emissions_data(company_id, reporting_year)
            historical_data = self._get_historical_emissions_data(company_id, reporting_year)
            
            if not current_year_data:
                logger.warning(f"No emissions data found for company {company_id} in year {reporting_year}")
            ...
            logger.info(f"Anomaly detection completed. Found {len(detected_anomalies)} anomalies")
            return report
    ...
    def _get_emissions_data(self, company_id: UUID, year: int) -> List[EmissionsCalculation]:
        """Get emissions data for a specific company and year"""
        return self.db.query(EmissionsCalculation).filter(
            and_(
                EmissionsCalculation.company_id == company_id,
                func.extract('year', EmissionsCalculation.reporting_period_start) == year
            )
        ).all()
    ...
    def _get_historical_emissions_data(
        ...
        return self.db.query(EmissionsCalculation).filter(
            and_(
                EmissionsCalculation.company_id == company_id,
                func.extract('year', EmissionsCalculation.reporting_period_start) >= start_year,
                func.extract('year', EmissionsCalculation.reporting_period_start) < current_year
            )
        ).order_by(EmissionsCalculation.reporting_period_start).all()
    ...
    def _detect_year_over_year_variance(
        ...
        previous_year_data = [d for d in historical_data if d.reporting_period_start.year == 
        ...
        if prev_scope1 > 0:
            ...
            if scope1_variance > self.year_over_year_threshold:
                severity = self._calculate_severity(scope1_variance, self.year_over_year_threshold)
        ...
        if prev_scope2 > 0:
            ...
            if scope2_variance > self.year_over_year_threshold:
                severity = self._calculate_severity(scope2_variance, self.year_over_year_threshold)
                anomalies.append(AnomalyDetectionResult(
                    anomaly_type=AnomalyType.YEAR_OVER_YEAR_VARIANCE,
                    severity=severity,
    ...
    def _detect_statistical_outliers(
        ...
        if len(historical_data) < 3:  # Need at least 3 data points for statistical analysis
            return anomalies
        ...
        for calc in current_data:
            ...
            if month in monthly_scope1 and len(monthly_scope1[month]) > 3:
                historical_values = [v for v in monthly_scope1[month] if v != (calc.scope1_emissions or 0)]
                if historical_values:
                    ...
                    if std_val > 0:
                        ...
                        if z_score > self.statistical_outlier_threshold:
                            severity = self._calculate_severity(z_score, self.statistical_outlier_threshold)
                            anomalies.append(AnomalyDetectionResult(
    ...
    def _detect_industry_benchmark_deviations(
        ...
        if scope1_deviation > self.industry_benchmark_threshold:
            severity = self._calculate_severity(scope1_deviation, self.industry_benchmark_threshold)
            anomalies.append(AnomalyDetectionResult(
        ...
        if scope2_deviation > self.industry_benchmark_threshold:
            severity = self._calculate_severity(scope2_deviation, self.industry_benchmark_threshold)
            anomalies.append(AnomalyDetectionResult(
                anomaly_type=AnomalyType.INDUSTRY_BENCHMARK_DEVIATION,
                severity=severity,
    ...
    def _detect_operational_inconsistencies(
        ...
        for calc in current_data:
            ...
            if calc.scope2_emissions and calc.scope2_emissions > 0:
                electricity_consumption = activity_data.get("electricity_consumption", 0)
    ...
    def _calculate_overall_risk_score(self, anomalies: List[AnomalyDetectionResult]) -> float:
    ...
    def _generate_summary_insights(self, anomalies: List[AnomalyDetectionResult]) -> List[str]:
        ...
        if AnomalyType.YEAR_OVER_YEAR_VARIANCE in type_counts:
            insights.append(f"Detected {type_counts[AnomalyType.YEAR_OVER_YEAR_VARIANCE]} significant year-over-year variance(s) - review operational changes")
        
        if AnomalyType.STATISTICAL_OUTLIER in type_counts:
            insights.append(f"Found {type_counts[AnomalyType.STATISTICAL_OUTLIER]} statistical outlier(s) - verify data accuracy")
        
        if AnomalyType.INDUSTRY_BENCHMARK_DEVIATION in type_counts:
            insights.append(f"Identified {type_counts[AnomalyType.INDUSTRY_BENCHMARK_DEVIATION]} industry benchmark deviation(s) - consider peer benchmarking")
        
        if AnomalyType.OPERATIONAL_DATA_INCONSISTENCY in type_counts:
            insights.append(f"Found {type_counts[AnomalyType.OPERATIONAL_DATA_INCONSISTENCY]} operational data inconsistency(ies) - review data collection processes")
    ...
    def _create_empty_report(self, company_id: UUID, reporting_year: int) -> AnomalyReport:
