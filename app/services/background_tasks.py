...
class BackgroundTaskManager:
    ...
    async def start_all_tasks(self):
        ...
        self.tasks["epa_refresh"] = asyncio.create_task(
            self._epa_refresh_task()
        )
    ...
    async def _epa_refresh_task(self):
        ...
        while True:
            try:
                ...
                try:
                    async with EPACachedService(db) as epa_service:
                        ...
                        if refresh_results["overall_status"] == "success":
                            ...
                            logger.warning(f"EPA refresh completed with issues: {refresh_results}")
                ...
                await self._send_notification(
                    "EPA Refresh Error",
                    f"EPA data refresh failed: {str(e)}"
                )
    ...
    async def _cleanup_stale_cache(self, epa_service: EPACachedService):
        ...
        try:
            ...
            for source, status in cache_status.get("cache", {}).get("sources", {}).items():
                if status.get("cached_at"):
                    ...
                    if age_days > 30:  # Older than 30 days
                        logger.warning(f"Very old cache detected for {source}: {age_days} days")
