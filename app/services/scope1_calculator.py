...
class Scope1EmissionsCalculator:
    ...
    async def _get_emission_factor(self, activity_type: str, fuel_type: Optional[str]) -> Optional[EmissionFactor]:
    ...
    async def _select_best_emission_factor(
        ...
        sources_to_try = factor_priorities.get(activity_type, {}).get(fuel_type, ["EPA_GHGRP", "EPA_AP42"])
        ...
        for source in sources_to_try:
            try:
                ...
                if factors:
                    ...
                    best_factor = self._rank_emission_factors(factors, activity_type, fuel_type)
                    if best_factor:
                        ...
                        if db_factor:
                            logger.info(f"Selected emission factor: {db_factor.factor_code} from {source}")
                            return db_factor
    ...
    def _rank_emission_factors(self, factors: List, activity_type: str, fuel_type: Optional[str]):
        ...
        for factor in factors:
            ...
            if fuel_type and hasattr(factor, 'fuel_type') and factor.fuel_type == fuel_type:
                score += 100
    ...
    async def _fallback_factor_selection(self, activity_type: str, fuel_type: Optional[str]) -> Optional[EmissionFactor]:
        ...
        try:
            ...
            fallback_factor = self.db.query(EmissionFactor).filter(
                EmissionFactor.is_current == True
            ).order_by(EmissionFactor.publication_year.desc()).first()
            
            if fallback_factor:
                logger.warning(f"Using last resort factor: {fallback_factor.factor_code}")
    ...
    def _convert_units(self, quantity: float, from_unit: str, to_unit: str) -> float:
        ...
        converted_value = self._try_multi_step_conversion(quantity, from_unit_norm, to_unit_norm, conversion_factors)
        ...
        for category, intermediates in intermediate_units.items():
            for intermediate in intermediates:
                ...
                if step1_key in conversion_factors and step2_key in conversion_factors:
                    ...
                    logger.debug(f"Multi-step conversion: {quantity} {from_unit} -> {intermediate_value} {intermediate} -> {final_value} {to_unit}")
                    return final_value
        
        return None
    
    async def _validate_calculation_request(self, request: Scope1CalculationRequest) -> CalculationValidationResult:
        ...
        for i, activity in enumerate(request.activity_data):
            ...
            factor = await self._get_emission_factor(activity.activity_type, activity.fuel_type)
            if not factor:
    ...
    def _calculate_data_quality_score(self, activity_data: List[ActivityDataInput]) -> float:
        ...
        for activity in activity_data:
            ...
            if activity.data_source:
                if 'meter' in activity.data_source.lower() or 'invoice' in activity.data_source.lower():
            ...
            if activity.activity_period_start and activity.activity_period_end:
                period_days = (activity.activity_period_end - activity.activity_period_start).days
    ...
    def _generate_recommendations(self, activity_data: List[ActivityDataInput]) -> List[str]:
        ...
        if estimated_count > 0:
            recommendations.append(f"🎯 Improve {estimated_count} estimated data points with measured values to increase accuracy by up to 40%")
        
        # Data source recommendations
        missing_sources = sum(1 for activity in activity_data if not activity.data_source)
        if missing_sources > 0:
            recommendations.append(f"📋 Add data sources for {missing_sources} activity items for better audit trail")
        
        # Location specificity
        missing_locations = sum(1 for activity in activity_data if not activity.location)
        if missing_locations > 0:
            recommendations.append(f"📍 Specify locations for {missing_locations} activities for regional emission factor accuracy")
        
        # Measurement method improvements
        missing_methods = sum(1 for activity in activity_data if not activity.measurement_method)
        if missing_methods > 0:
            recommendations.append(f"🔬 Define measurement methods for {missing_methods} activities to improve data quality score")
        
        # Time period granularity
        annual_periods = sum(1 for activity in activity_data 
                           if activity.activity_period_start and activity.activity_period_end 
                           and (activity.activity_period_end - activity.activity_period_start).days > 365)
        if annual_periods > 0:
            recommendations.append(f"📅 Consider breaking down {annual_periods} annual periods into monthly/quarterly data for better accuracy")
        
        # Large emission sources
        if activity_data:
            sorted_activities = sorted(activity_data, key=lambda x: x.quantity, reverse=True)
            ...
            for i, activity in enumerate(top_activities):
                if activity.data_quality == 'estimated':
                    recommendations.append(f"⚡ High-impact activity #{i+1} ({activity.activity_type}) uses estimated data - prioritize measurement for maximum accuracy improvement")
        
        # Fuel type diversity
        fuel_types = set(activity.fuel_type for activity in activity_data if activity.fuel_type)
        if len(fuel_types) > 5:
            recommendations.append("🔄 Consider consolidating similar fuel types or implementing fuel-specific tracking systems")
        
        # Missing documentation
        missing_docs = sum(1 for activity in activity_data if not activity.notes and not activity.additional_data)
        if missing_docs > 0:
            recommendations.append(f"📝 Add documentation/notes for {missing_docs} activities to support audit requirements")
        
        # EPA factor availability
        recommendations.append("✅ Verify EPA emission factors are current - system will automatically use latest available factors")
        
        # SEC compliance
        recommendations.append("🏛️ Ensure all data sources are auditable and documentation is SEC-compliant for climate disclosure requirements")
    ...
    def _generate_calculation_insights(
        ...
        insights["summary"] = {
            ...
            "average_co2e_per_activity": round(total_co2e / len(activity_data), 2) if activity_data else 0,
            "fuel_types_count": len(set(a.fuel_type for a in activity_data if a.fuel_type)),
            "locations_count": len(set(a.location for a in activity_data if a.location)),
            "data_sources_count": len(set(a.data_source for a in activity_data if a.data_source))
        }
        ...
        for activity in activity_data:
            ...
            fuel_breakdown[fuel_type]["estimated_co2e"] += activity.quantity * 0.05  # Placeholder
        ...
        insights["quality_analysis"] = {
            ...
            "measured_percentage": round((quality_distribution.get("measured", 0) / len(activity_data)) * 100, 1) if activity_data else 0,
            "estimated_percentage": round((quality_distribution.get("estimated", 0) / len(activity_data)) * 100, 1) if activity_data else 0,
        ...
        insights["benchmarks"] = {
            "emissions_intensity": {
                "co2e_per_activity": round(total_co2e / len(activity_data), 2) if activity_data else 0,
                "benchmark_category": self._categorize_emissions_intensity(total_co2e, len(activity_data))
            },
            "data_quality_rating": self._rate_data_quality(insights["quality_analysis"]["overall_quality_score"]),
            "completeness_rating": self._rate_completeness(insights["quality_analysis"]["data_completeness_score"])
        }
        
        # EPA factors used summary
        insights["epa_factors"] = {
            "factors_used_count": len(emission_factors_used),
            "sources": list(set(factor.get("factor_source", "unknown") for factor in emission_factors_used.values())),
            "latest_factor_year": max([2024] + [2020])  # Placeholder - would extract from actual factors
        }
        
        return insights
    
    def _categorize_emissions_intensity(self, total_co2e: float, activity_count: int) -> str:
