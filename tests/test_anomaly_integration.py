...
from app.schemas.anomaly_detection import AnomalyReportResponse, AnomalyDetectionResultResponse
...
class TestAnomalyIntegration:
        ...
        return AnomalyReportResponse(
            ...
            detected_anomalies=[
                AnomalyDetectionResultResponse(
                    ...
                    recommendations=["Review fuel consumption data", "Verify calculation methods"],
                ...
                AnomalyDetectionResultResponse(
                    ...
                    recommendations=["Check meter readings", "Validate reporting period"],
    ...
    async def test_validation_service_anomaly_integration(self, mock_db, sample_anomaly_report):
        ...
        with patch('app.services.emissions_validation_service.AnomalyDetectionService') as mock_anomaly_service:
            ...
            with patch('app.services.emissions_validation_service.EPAGHGRPService') as mock_epa_service:
    ...
    def test_audit_service_anomaly_integration(self, mock_db, sample_anomaly_report):
        ...
        # Mock the anomaly detection service
        with patch('app.services.enhanced_audit_service.AnomalyDetectionService') as mock_anomaly_service:
            mock_anomaly_instance = Mock()
            mock_anomaly_instance.detect_anomalies.return_value = sample_anomaly_report
            mock_anomaly_service.return_value = mock_anomaly_instance
    ...
    def test_anomaly_detection_error_handling(self, mock_db):
        ...
        with patch('app.services.emissions_validation_service.AnomalyDetectionService') as mock_anomaly_service:
            mock_anomaly_instance = Mock()
            mock_anomaly_instance.detect_anomalies.side_effect = Exception("Anomaly detection failed")
            mock_anomaly_service.return_value = mock_anomaly_instance
    ...
    async def test_end_to_end_anomaly_workflow(self, mock_db, sample_anomaly_report):
        ...
        with patch.object(AnomalyDetectionService, 'detect_anomalies', return_value=sample_anomaly_report):
            ...
            with patch('app.services.enhanced_audit_service.AnomalyDetectionService') as mock_audit_anomaly:
