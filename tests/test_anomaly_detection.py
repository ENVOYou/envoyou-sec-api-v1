...
async def test_anomaly_detection_service():
    ...
    mock_db.query.return_value.filter.return_value.order_by.return_value.all.return_value = mock_historical_data
    ...
    with patch.object(service, '_get_emissions_data', return_value=mock_current_data):
        with patch.object(service, '_get_historical_emissions_data', return_value=mock_historical_data):
...
def test_year_over_year_variance_detection():
    ...
    current_data = [
        Mock(scope1_emissions=1200.0, scope2_emissions=600.0)
    ]
...
def test_operational_data_inconsistency():
    ...
    current_data = [
        Mock(
            scope1_emissions=1000.0,
            scope2_emissions=500.0,
            activity_data={"fuel_consumption": 0, "electricity_consumption": 0}  # Inconsistent
        )
    ]
    
    historical_data = []
    
    anomalies = service._detect_operational_inconsistencies(current_data, historical_data)
    ...
    for anomaly in anomalies:
        ...
        assert any(word in anomaly.description.lower() for word in ["inconsistency", "reported but no", "data"])
