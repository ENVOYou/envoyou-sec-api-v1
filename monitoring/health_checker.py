...
class HealthChecker:
    ...
    def check_external_dependencies(self) -> List[Dict]:
        ...
        try:
            cf_response = self.session.get('https://www.cloudflarestatus.com/', timeout=10)
            results.append({
                'component': 'cloudflare',
                'status': 'healthy' if cf_response.status_code == 200 else 'unhealthy',
        ...
        try:
            ...
            results.append({
                ...
                'status': 'healthy' if neon_response.status_code in [200, 301] else 'unhealthy',
        ...
        try:
            upstash_response = self.session.get('https://status.upstash.com/', timeout=10)
            results.append({
                'component': 'upstash_redis',
                'status': 'healthy' if upstash_response.status_code == 200 else 'unhealthy',
    ...
    def _send_email_alert(self, alert_data: Dict):
        ...
        try:
            ...
            msg['Subject'] = f"ENVOYOU Health Alert: {alert_data['overall_status'].upper()}"
    ...
    def run_health_checks(self):
        ...
        while True:
            try:
                ...
                logger.info(f"{status_emoji} Health Status: {analysis['overall_status'].upper()}")
                ...
                if analysis['overall_status'] in ['unhealthy', 'warning'] and analysis['issues']:
