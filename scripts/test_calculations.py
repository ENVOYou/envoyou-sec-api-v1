...
class CalculationTester:
    ...
    def test_scope1_calculation(self):
        ...
        print(f"📊 Input Data:")
        print(f"   Natural Gas Boiler #1: {request.activity_data[0].quantity:,} {request.activity_data[0].unit}")
        print(f"   Natural Gas Boiler #2: {request.activity_data[1].quantity:,} {request.activity_data[1].unit}")
        print(f"   Diesel Fleet: {request.activity_data[2].quantity:,} {request.activity_data[2].unit}")
        print()
        
        try:
            # Perform calculation
            result = calculator.calculate_scope1_emissions(request, str(self.test_user.id))
            ...
            print(f"   Processing Time: {result.calculation_duration_seconds:.3f} seconds")
            print()
            
            print("📋 Activity Breakdown:")
            for i, activity in enumerate(result.activity_data, 1):
                print(f"   {i}. {activity['activity_description']}")
                print(f"      Emissions: {activity['co2e_emissions']:,.2f} metric tons CO2e")
                print(f"      Factor: {activity['emission_factor_value']} {activity['emission_factor_unit']}")
    ...
    def test_scope2_calculation(self):
        ...
        print(f"📊 Input Data:")
        print(f"   Main Facility: {request.electricity_consumption[0].quantity:,} {request.electricity_consumption[0].unit}")
        print(f"   Office Building: {request.electricity_consumption[1].quantity:,} {request.electricity_consumption[1].unit}")
        print(f"   Method: {request.calculation_method}")
        print()
        
        try:
            # Perform calculation
            result = calculator.calculate_scope2_emissions(request, str(self.test_user.id))
            ...
            for i, activity in enumerate(result.activity_data, 1):
                ...
                print(f"      Emissions: {activity['co2e_emissions']:,.2f} metric tons CO2e")
                print(f"      Factor: {activity['emission_factor_value']} {activity['emission_factor_unit']}")
                print(f"      Region: {activity.get('additional_data', {}).get('electricity_region', 'N/A')}")
                print()
    ...
    def test_audit_trail(self, calculation_result):
        ...
        try:
            # Get audit trail
            audit_trail = audit_service.get_calculation_audit_trail(calculation_result.id)
            ...
            integrity = audit_service.verify_calculation_integrity(calculation_result.id)
            ...
            print(f"   SEC Compliant: {'✅ Yes' if forensic_report['compliance_attestation']['sec_climate_disclosure_compliant'] else '❌ No'}")
            print(f"   Audit Trail Complete: {'✅ Yes' if forensic_report['compliance_attestation']['audit_trail_complete'] else '❌ No'}")
            print(f"   Data Traceable: {'✅ Yes' if forensic_report['compliance_attestation']['data_traceable'] else '❌ No'}")
            print(f"   Calculation Reproducible: {'✅ Yes' if forensic_report['compliance_attestation']['calculation_reproducible'] else '❌ No'}")
            print()
    ...
    def test_company_summary(self):
        ...
        try:
            summary = audit_service.get_company_audit_summary(str(self.test_company.id), 2023)
