...
def test_calculation_schemas():
    ...
    try:
        ...
        print(
            f"   Quantity: {scope1_request.activity_data[0].quantity} {scope1_request.activity_data[0].unit}"
        )
        print()
        ...
        print(
            f"   Quantity: {scope2_request.electricity_consumption[0].quantity} {scope2_request.electricity_consumption[0].unit}"
        )
        print()
...
def test_calculation_logic():
    ...
    try:
        ...
        print(
            f"   Natural Gas: {ng_quantity} MMBtu × {ng_factor} kg CO2e/MMBtu = {ng_emissions:.2f} tCO2e"
        )
        ...
        print(
            f"   Electricity: {elec_quantity} MWh × {elec_factor} kg CO2e/MWh = {elec_emissions:.2f} tCO2e"
        )
        print()
        ...
        total_co2e = (
            co2_emissions * gwp_co2 + ch4_emissions * gwp_ch4 + n2o_emissions * gwp_n2o
        )

        print(
            f"   CO2: {co2_emissions} t × {gwp_co2} = {co2_emissions * gwp_co2:.2f} tCO2e"
        )
        print(
            f"   CH4: {ch4_emissions} t × {gwp_ch4} = {ch4_emissions * gwp_ch4:.2f} tCO2e"
        )
        print(
            f"   N2O: {n2o_emissions} t × {gwp_n2o} = {n2o_emissions * gwp_n2o:.2f} tCO2e"
        )
...
def test_data_quality_scoring():
    ...
    try:
        ...
        quality_scores = {"measured": 100, "calculated": 80, "estimated": 60}
    ...
    try:
        ...
        for quantity, from_unit, to_unit in test_cases:
            ...
            if conversion_key in conversions:
                ...
                print(
                    f"   {quantity} {from_unit} = {quantity} {to_unit} (no conversion)"
                )
...
def test_sec_compliance_features():
    ...
    try:
        ...
        audit_event = {
            ...
            "data_changed": {"total_co2e": 53.11, "status": "completed"},
        }
        ...
        print(
            f"   SEC Compliant: {'✅ Yes' if forensic_report['sec_compliant'] else '❌ No'}"
        )
        print(
            f"   Audit Trail Complete: {'✅ Yes' if forensic_report['audit_trail_complete'] else '❌ No'}"
        )
        print(
            f"   Data Traceable: {'✅ Yes' if forensic_report['data_traceable'] else '❌ No'}"
        )
        print(
            f"   Calculation Reproducible: {'✅ Yes' if forensic_report['calculation_reproducible'] else '❌ No'}"
        )
